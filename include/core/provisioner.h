...
class IProvisioner {
 public:
  ...
  // First-boot welcome screen: shown once on a unit with no machine and no
  // WiFi saved, until dismissed (persisted). A restored backup carries the
  // saved machine/WiFi along, so a replacement unit never sees it.
