 ...
 public:
  void series(uint32_t /*window_s*/, float* brew, float* boiler, int n) const override {
    for (int i = 0; i < n; ++i) {
      ...
      // Two no-data spans so the Stats greying is visible: a short leading span
      // (window predates recording) and a mid-stream gap (BLE dropped while the
      // device kept running).
      if (frac < 0.12f || (frac > 0.5f && frac < 0.64f)) {
        brew[i] = NAN;
        boiler[i] = NAN;
        continue;
      }
